...
def sentiment_agg(avg_info,tmp,name,agg_type='skin type'):
    ...
    avg_info[col_sum] = avg_info['{}_ratio'.format(name)] * avg_info['{}_wilson'.format(name)] * (1/(1+np.exp(-np.log10(avg_info['{}_n_pos'.format(name)]))))
...
def gen_knowledge_model(file, bert_path):
	...
	df_sum.to_csv('kbm.csv', index=False)
